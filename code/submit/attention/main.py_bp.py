...
if __name__ == '__main__':
    ...
    attn = model.Decomposable_Attention(args)
    ...
    if args.snapshot is not None:
        ...
        attn.load_state_dict(torch.load(args.snapshot))
        if args.cuda:
            torch.cuda.set_device(args.device)
            attn = attn.cuda()
        ...
        try:
            train(train_iter=train_iter, dev_iter=dev_iter, model=attn, args=args)
        except KeyboardInterrupt:
            print(traceback.print_exc())
            print('\n' + '-' * 89)
            print('Exiting from training early')

    test(test_iter=test_iter, model=attn, args=args)
