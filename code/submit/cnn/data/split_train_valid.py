...
valid_df.to_csv("valid_{}.tsv".format(valid_sample_num), sep="\t", index=False, encoding="utf-8", header=False)


train_df = data_df[~data_df.isin(valid_df)]
train_df.dropna(inplace=True)
train_df['id'] = train_df['id'].apply(lambda x: int(float(re.sub('\xef\xbb\xbf', '', x)) ))
...
valid_df.to_csv("test_{}.tsv".format(valid_sample_num), sep="\t", encoding="utf-8", header=False, index=False)
