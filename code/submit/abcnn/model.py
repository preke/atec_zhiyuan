...
class Abcnn3(nn.Module):
    ...
    '''
    def __init__(self, emb_dim, emb_num, sentence_length, filter_width, filter_channel=300, layer_size=2, match='cosine', inception=True, pretrained_weight=None):
...
def attention_matrix(x1, x2, eps=1e-6):
    ...
    '''
    eps = torch.tensor(eps).to(device='cuda:0')
    one = torch.tensor(1.).to(device='cuda:0')
